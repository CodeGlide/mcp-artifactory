...
func Actions_update_repo_variableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/variables/%s", cfg.BaseURL, owner, repo, name)
...
func CreateActions_update_repo_variableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_actions_variables_name",
		mcp.WithDescription("Update a repository variable"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_update_repo_variableHandler(cfg),
	}
}
