...
func Put_networks_networkid_appliance_singlelanHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_appliance_singlelanTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_appliance_singleLan",
		mcp.WithDescription("Update Single LAN Configuration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_appliance_singlelanHandler(cfg),
	}
}
