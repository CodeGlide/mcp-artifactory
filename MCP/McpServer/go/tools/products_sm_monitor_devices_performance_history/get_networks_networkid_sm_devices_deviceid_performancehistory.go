...
func Get_networks_networkid_sm_devices_deviceid_performancehistoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/sm/devices/%s/performanceHistory", cfg.BaseURL, networkId, deviceId)
...
func CreateGet_networks_networkid_sm_devices_deviceid_performancehistoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_sm_devices_deviceId_performanceHistory",
		mcp.WithDescription("Return historical records of various Systems Manager client metrics for desktop devices."),
		mcp.WithString("X-Cisco-Meraki-API-Key", mcp.Description("")),
		mcp.WithString("networkId", mcp.Required(), mcp.Description("(Required) ")),
		mcp.WithString("deviceId", mcp.Required(), mcp.Description("(Required) ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_sm_devices_deviceid_performancehistoryHandler(cfg),
	}
}
