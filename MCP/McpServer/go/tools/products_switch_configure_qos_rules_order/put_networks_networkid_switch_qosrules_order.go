...
func Put_networks_networkid_switch_qosrules_orderHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_switch_qosrules_orderTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_switch_qosRules_order",
		mcp.WithDescription("Update The Order In Which The Rules Should Be Processed By The Switch"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_switch_qosrules_orderHandler(cfg),
	}
}
