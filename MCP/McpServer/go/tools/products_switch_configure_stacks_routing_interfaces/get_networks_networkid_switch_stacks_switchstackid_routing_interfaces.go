...
func Get_networks_networkid_switch_stacks_switchstackid_routing_interfacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_stacks_switchstackid_routing_interfacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_stacks_switchStackId_routing_interfaces",
		mcp.WithDescription("List Layer 3 Interfaces For A Switch Stack"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_stacks_switchstackid_routing_interfacesHandler(cfg),
	}
}
