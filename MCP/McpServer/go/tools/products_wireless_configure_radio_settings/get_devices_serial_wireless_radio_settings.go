...
func Get_devices_serial_wireless_radio_settingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_devices_serial_wireless_radio_settingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_devices_serial_wireless_radio_settings",
		mcp.WithDescription("Return The Radio Settings Of A Device"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_devices_serial_wireless_radio_settingsHandler(cfg),
	}
}
