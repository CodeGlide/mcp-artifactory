...
func Get_devices_serial_appliance_performanceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/devices/%s/appliance/performance", cfg.BaseURL, serial)
...
func CreateGet_devices_serial_appliance_performanceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_devices_serial_appliance_performance",
		mcp.WithDescription("Return the performance score for a single MX"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_devices_serial_appliance_performanceHandler(cfg),
	}
}
