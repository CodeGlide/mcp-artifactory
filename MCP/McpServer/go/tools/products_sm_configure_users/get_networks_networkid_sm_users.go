...
func Get_networks_networkid_sm_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/sm/users", cfg.BaseURL, networkId)
...
func CreateGet_networks_networkid_sm_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_sm_users",
		mcp.WithDescription("List The Owners In An SM Network With Various Specified Fields And Filters"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_sm_usersHandler(cfg),
	}
}
