...
func Get_networks_networkid_switch_dscptocosmappingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_dscptocosmappingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_dscpToCosMappings",
		mcp.WithDescription("Return The DSCP To Co S Mappings"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_dscptocosmappingsHandler(cfg),
	}
}
