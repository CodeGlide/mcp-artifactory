...
func Get_networks_networkid_wireless_ssids_number_bonjourforwardingHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_wireless_ssids_number_bonjourforwardingTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_wireless_ssids_number_bonjourForwarding",
		mcp.WithDescription("List The Bonjour Forwarding Setting And Rules For The SSID"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_wireless_ssids_number_bonjourforwardingHandler(cfg),
	}
}
