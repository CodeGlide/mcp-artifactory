...
func Get_networks_networkid_switch_linkaggregationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_linkaggregationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_linkAggregations",
		mcp.WithDescription("List Link Aggregation Groups"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_linkaggregationsHandler(cfg),
	}
}
