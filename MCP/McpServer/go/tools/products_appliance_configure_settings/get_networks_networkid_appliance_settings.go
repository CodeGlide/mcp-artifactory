...
func Get_networks_networkid_appliance_settingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/appliance/settings", cfg.BaseURL, networkId)
...
func CreateGet_networks_networkid_appliance_settingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_appliance_settings",
		mcp.WithDescription("Return the appliance settings for a network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_appliance_settingsHandler(cfg),
	}
}
