...
func Get_v1_teams_team_name_gateways_gateway_id_statusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/gateways/%s/status", cfg.BaseURL, team_name, gateway_id)
...
func CreateGet_v1_teams_team_name_gateways_gateway_id_statusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_gateways_gateway_id_status",
		mcp.WithDescription("Retrieve a status report for a Gateway"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_gateways_gateway_id_statusHandler(cfg),
	}
}
