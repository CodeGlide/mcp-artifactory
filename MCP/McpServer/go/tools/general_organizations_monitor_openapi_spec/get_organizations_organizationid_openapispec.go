...
func Get_organizations_organizationid_openapispecHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/organizations/%s/openapiSpec", cfg.BaseURL, organizationId)
...
func CreateGet_organizations_organizationid_openapispecTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_organizations_organizationId_openapiSpec",
		mcp.WithDescription("Return the OpenAPI 2.0 Specification of the organization's API documentation in JSON"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_organizations_organizationid_openapispecHandler(cfg),
	}
}
