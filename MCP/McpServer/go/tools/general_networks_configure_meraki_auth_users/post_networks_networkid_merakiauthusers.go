...
func Post_networks_networkid_merakiauthusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/merakiAuthUsers", cfg.BaseURL, networkId)
...
func CreatePost_networks_networkid_merakiauthusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_networks_networkId_merakiAuthUsers",
		mcp.WithDescription("Authorize A User Configured With Meraki Authentication For A Network Currently Supports 802 1 X Splash Guest And Client VPN Users And Currently Organizations Have A 50 000 User Cap"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_networks_networkid_merakiauthusersHandler(cfg),
	}
}
