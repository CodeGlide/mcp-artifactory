...
func Post_devices_serial_switch_routing_staticroutesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePost_devices_serial_switch_routing_staticroutesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_devices_serial_switch_routing_staticRoutes",
		mcp.WithDescription("Create A Layer 3 Static Route For A Switch"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_devices_serial_switch_routing_staticroutesHandler(cfg),
	}
}
