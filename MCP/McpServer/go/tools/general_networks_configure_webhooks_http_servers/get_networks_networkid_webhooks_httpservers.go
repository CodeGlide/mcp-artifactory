...
func Get_networks_networkid_webhooks_httpserversHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_webhooks_httpserversTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_webhooks_httpServers",
		mcp.WithDescription("List The HTTP Servers For A Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_webhooks_httpserversHandler(cfg),
	}
}
