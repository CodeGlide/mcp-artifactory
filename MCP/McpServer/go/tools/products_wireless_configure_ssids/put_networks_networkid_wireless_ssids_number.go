...
func Put_networks_networkid_wireless_ssids_numberHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_wireless_ssids_numberTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_wireless_ssids_number",
		mcp.WithDescription("Update The Attributes Of An MR SSID"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_wireless_ssids_numberHandler(cfg),
	}
}
