...
func Get_networks_networkid_splashloginattemptsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/splashLoginAttempts", cfg.BaseURL, networkId)
...
func CreateGet_networks_networkid_splashloginattemptsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_splashLoginAttempts",
		mcp.WithDescription("List the splash login attempts for a network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_splashloginattemptsHandler(cfg),
	}
}
