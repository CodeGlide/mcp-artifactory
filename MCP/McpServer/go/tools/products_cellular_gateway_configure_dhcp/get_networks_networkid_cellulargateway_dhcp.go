...
func Get_networks_networkid_cellulargateway_dhcpHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_cellulargateway_dhcpTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_cellularGateway_dhcp",
		mcp.WithDescription("List Common DHCP Settings Of M Gs"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_cellulargateway_dhcpHandler(cfg),
	}
}
