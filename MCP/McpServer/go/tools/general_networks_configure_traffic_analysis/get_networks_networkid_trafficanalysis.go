...
func Get_networks_networkid_trafficanalysisHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_trafficanalysisTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_trafficAnalysis",
		mcp.WithDescription("Return The Traffic Analysis Settings For A Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_trafficanalysisHandler(cfg),
	}
}
