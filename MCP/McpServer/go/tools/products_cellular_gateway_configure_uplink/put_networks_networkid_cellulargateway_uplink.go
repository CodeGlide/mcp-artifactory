...
func Put_networks_networkid_cellulargateway_uplinkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_cellulargateway_uplinkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_cellularGateway_uplink",
		mcp.WithDescription("Updates The Uplink Settings For Your MG Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_cellulargateway_uplinkHandler(cfg),
	}
}
