...
func Get_networks_networkid_wireless_airmarshalHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/wireless/airMarshal", cfg.BaseURL, networkId)
...
func CreateGet_networks_networkid_wireless_airmarshalTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_wireless_airMarshal",
		mcp.WithDescription("List Air Marshal scan results from a network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_wireless_airmarshalHandler(cfg),
	}
}
