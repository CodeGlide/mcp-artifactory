...
func Get_networks_networkid_sm_users_userid_deviceprofilesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/sm/users/%s/deviceProfiles", cfg.BaseURL, networkId, userId)
...
func CreateGet_networks_networkid_sm_users_userid_deviceprofilesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_sm_users_userId_deviceProfiles",
		mcp.WithDescription("Get the profiles associated with a user"),
		mcp.WithString("X-Cisco-Meraki-API-Key", mcp.Description("")),
		mcp.WithString("networkId", mcp.Required(), mcp.Description("(Required) ")),
		mcp.WithString("userId", mcp.Required(), mcp.Description("(Required) ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_sm_users_userid_deviceprofilesHandler(cfg),
	}
}
