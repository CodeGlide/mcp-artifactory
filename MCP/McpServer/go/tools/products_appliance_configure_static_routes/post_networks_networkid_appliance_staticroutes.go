...
func Post_networks_networkid_appliance_staticroutesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePost_networks_networkid_appliance_staticroutesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_networks_networkId_appliance_staticRoutes",
		mcp.WithDescription("Add A Static Route For An MX Or Teleworker Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_networks_networkid_appliance_staticroutesHandler(cfg),
	}
}
