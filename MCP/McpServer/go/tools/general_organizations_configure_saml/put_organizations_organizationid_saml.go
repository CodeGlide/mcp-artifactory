...
func Put_organizations_organizationid_samlHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_organizations_organizationid_samlTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_organizations_organizationId_saml",
		mcp.WithDescription("Updates The SAML SSO Enabled Settings For An Organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_organizations_organizationid_samlHandler(cfg),
	}
}
