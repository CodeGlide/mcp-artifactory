...
func Get_networks_networkid_wireless_usagehistoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/wireless/usageHistory", cfg.BaseURL, networkId)
...
func CreateGet_networks_networkid_wireless_usagehistoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_wireless_usageHistory",
		mcp.WithDescription("Return AP usage over time for a device or network client"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_wireless_usagehistoryHandler(cfg),
	}
}
