...
func Put_devices_serial_cellulargateway_portforwardingrulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_devices_serial_cellulargateway_portforwardingrulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_devices_serial_cellularGateway_portForwardingRules",
		mcp.WithDescription("Updates The Port Forwarding Rules For A Single MG"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_devices_serial_cellulargateway_portforwardingrulesHandler(cfg),
	}
}
