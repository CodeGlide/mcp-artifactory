...
func Get_networks_networkid_switch_stacks_switchstackidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_stacks_switchstackidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_stacks_switchStackId",
		mcp.WithDescription("Show A Switch Stack"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_stacks_switchstackidHandler(cfg),
	}
}
