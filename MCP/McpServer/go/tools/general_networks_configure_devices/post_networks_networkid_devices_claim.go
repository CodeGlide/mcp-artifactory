...
func Post_networks_networkid_devices_claimHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/devices/claim", cfg.BaseURL, networkId)
...
func CreatePost_networks_networkid_devices_claimTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_networks_networkId_devices_claim",
		mcp.WithDescription("Claim Devices Into A Network Note For Recently Claimed Devices It May Take A Few Minutes For API Requsts Against That Device To Succeed"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_networks_networkid_devices_claimHandler(cfg),
	}
}
