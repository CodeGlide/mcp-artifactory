...
func Put_devices_serial_managementinterfaceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_devices_serial_managementinterfaceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_devices_serial_managementInterface",
		mcp.WithDescription("Update The Management Interface Settings For A Device"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_devices_serial_managementinterfaceHandler(cfg),
	}
}
