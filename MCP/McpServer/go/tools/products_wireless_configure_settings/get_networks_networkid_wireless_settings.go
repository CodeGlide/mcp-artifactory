...
func Get_networks_networkid_wireless_settingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_wireless_settingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_wireless_settings",
		mcp.WithDescription("Return The Wireless Settings For A Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_wireless_settingsHandler(cfg),
	}
}
