...
func Get_v1_teams_team_name_resource_groups_resource_group_id_projects_project_id_servers_server_id_server_accountsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		server_idVal, ok := args["server_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: server_id"), nil
		}
		server_id, ok := server_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: server_id"), nil
		}
		url := fmt.Sprintf("%s/v1/teams/%s/resource_groups/%s/projects/%s/servers/%s/server_accounts", cfg.BaseURL, team_name, resource_group_id, project_id, server_id)
...
func CreateGet_v1_teams_team_name_resource_groups_resource_group_id_projects_project_id_servers_server_id_server_accountsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_resource_groups_resource_group_id_projects_project_id_servers_server_id_server_accounts",
		mcp.WithDescription("List all Server Accounts for a Server"),
		...
		mcp.WithString("server_id", mcp.Required(), mcp.Description("(Required) The UUID of an enrolled Server")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_resource_groups_resource_group_id_projects_project_id_servers_server_id_server_accountsHandler(cfg),
	}
}
