...
func Get_v1_teams_team_name_settingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/settings", cfg.BaseURL, team_name)
...
func CreateGet_v1_teams_team_name_settingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_settings",
		mcp.WithDescription("Retrieve settings for a Team"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_settingsHandler(cfg),
	}
}
