...
func Get_v1_teams_team_name_rolesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/roles", cfg.BaseURL, team_name)
...
func CreateGet_v1_teams_team_name_rolesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_roles",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_rolesHandler(cfg),
	}
}
