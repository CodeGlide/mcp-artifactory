...
func Get_networks_networkid_firmwareupgradesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_firmwareupgradesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_firmwareUpgrades",
		mcp.WithDescription("Get Firmware Upgrade Information For A Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_firmwareupgradesHandler(cfg),
	}
}
