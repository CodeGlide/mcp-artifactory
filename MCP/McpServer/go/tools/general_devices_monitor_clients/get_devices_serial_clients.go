...
func Get_devices_serial_clientsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/devices/%s/clients", cfg.BaseURL, serial)
...
func CreateGet_devices_serial_clientsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_devices_serial_clients",
		mcp.WithDescription("List the clients of a device, up to a maximum of a month ago"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_devices_serial_clientsHandler(cfg),
	}
}
