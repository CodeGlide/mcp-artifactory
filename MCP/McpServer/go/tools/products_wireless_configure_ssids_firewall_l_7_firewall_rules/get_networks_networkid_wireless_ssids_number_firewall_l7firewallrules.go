...
func Get_networks_networkid_wireless_ssids_number_firewall_l7firewallrulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/wireless/ssids/%s/firewall/l7FirewallRules", cfg.BaseURL, networkId, number)
...
func CreateGet_networks_networkid_wireless_ssids_number_firewall_l7firewallrulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_wireless_ssids_number_firewall_l7FirewallRules",
		mcp.WithDescription("Return The L 7 Firewall Rules For An SSID On An MR Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_wireless_ssids_number_firewall_l7firewallrulesHandler(cfg),
	}
}
