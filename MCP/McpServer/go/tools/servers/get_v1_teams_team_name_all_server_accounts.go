...
func Get_v1_teams_team_name_all_server_accountsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/all_server_accounts", cfg.BaseURL, team_name)
...
func CreateGet_v1_teams_team_name_all_server_accountsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_all_server_accounts",
		mcp.WithDescription("List all Server Account Resources"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_all_server_accountsHandler(cfg),
	}
}
