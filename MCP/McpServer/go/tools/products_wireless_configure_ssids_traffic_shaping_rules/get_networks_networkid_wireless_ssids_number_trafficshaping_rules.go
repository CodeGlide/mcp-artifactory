...
func Get_networks_networkid_wireless_ssids_number_trafficshaping_rulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_wireless_ssids_number_trafficshaping_rulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_wireless_ssids_number_trafficShaping_rules",
		mcp.WithDescription("Display The Traffic Shaping Settings For A SSID On An MR Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_wireless_ssids_number_trafficshaping_rulesHandler(cfg),
	}
}
