...
func Get_networks_networkid_switch_dhcpserverpolicyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_dhcpserverpolicyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_dhcpServerPolicy",
		mcp.WithDescription("Return The DHCP Server Policy"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_dhcpserverpolicyHandler(cfg),
	}
}
