...
func Put_networks_networkid_appliance_vlans_settingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_appliance_vlans_settingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_appliance_vlans_settings",
		mcp.WithDescription("Enable Disable VLA Ns For The Given Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_appliance_vlans_settingsHandler(cfg),
	}
}
