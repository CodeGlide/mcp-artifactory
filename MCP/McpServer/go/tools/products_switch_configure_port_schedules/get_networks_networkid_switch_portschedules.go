...
func Get_networks_networkid_switch_portschedulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_portschedulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_portSchedules",
		mcp.WithDescription("List Switch Port Schedules"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_portschedulesHandler(cfg),
	}
}
