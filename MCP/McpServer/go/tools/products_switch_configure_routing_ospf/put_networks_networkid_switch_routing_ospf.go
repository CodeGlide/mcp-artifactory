...
func Put_networks_networkid_switch_routing_ospfHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/switch/routing/ospf", cfg.BaseURL, networkId)
...
func CreatePut_networks_networkid_switch_routing_ospfTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_switch_routing_ospf",
		mcp.WithDescription("Update Layer 3 OSPF Routing Configuration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_switch_routing_ospfHandler(cfg),
	}
}
