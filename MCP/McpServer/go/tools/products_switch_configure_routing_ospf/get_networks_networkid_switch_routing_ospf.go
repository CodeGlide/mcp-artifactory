...
func Get_networks_networkid_switch_routing_ospfHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_routing_ospfTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_routing_ospf",
		mcp.WithDescription("Return Layer 3 OSPF Routing Configuration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_routing_ospfHandler(cfg),
	}
}
