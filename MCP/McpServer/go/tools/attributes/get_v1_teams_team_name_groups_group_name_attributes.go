...
func Get_v1_teams_team_name_groups_group_name_attributesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["conflicting"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("conflicting=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v1/teams/%s/groups/%s/attributes%s", cfg.BaseURL, team_name, group_name, queryString)
...
func CreateGet_v1_teams_team_name_groups_group_name_attributesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_groups_group_name_attributes",
		mcp.WithDescription("List all Attributes for a Group"),
		...
		mcp.WithString("conflicting", mcp.Description("When `true`, only return attributes that conflict with other attributes on your Team")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_groups_group_name_attributesHandler(cfg),
	}
}
