...
func Delete_api_services_jira_links_link_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_api_services_jira_links_link_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_services_jira_links_link_id",
		mcp.WithDescription("Delete Link"),
		mcp.WithString("link_id", mcp.Required(), mcp.Description("(Required) The id of the link")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_api_services_jira_links_link_idHandler(cfg),
	}
}
