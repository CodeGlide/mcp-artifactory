...
func Post_organizations_organizationid_switch_devices_cloneHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/organizations/%s/switch/devices/clone", cfg.BaseURL, organizationId)
...
func CreatePost_organizations_organizationid_switch_devices_cloneTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_organizations_organizationId_switch_devices_clone",
		mcp.WithDescription("Clone port-level and some switch-level configuration settings from a source switch to one or more target switches"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_organizations_organizationid_switch_devices_cloneHandler(cfg),
	}
}
