...
func Get_networks_networkid_sm_devices_deviceid_connectivityHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/sm/devices/%s/connectivity", cfg.BaseURL, networkId, deviceId)
...
func CreateGet_networks_networkid_sm_devices_deviceid_connectivityTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_sm_devices_deviceId_connectivity",
		mcp.WithDescription("Returns historical connectivity data (whether a device is regularly checking in to Dashboard)."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_sm_devices_deviceid_connectivityHandler(cfg),
	}
}
