...
func Post_devices_serial_switch_routing_interfacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/devices/%s/switch/routing/interfaces", cfg.BaseURL, serial)
...
func CreatePost_devices_serial_switch_routing_interfacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_devices_serial_switch_routing_interfaces",
		mcp.WithDescription("Create A Layer 3 Interface For A Switch"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_devices_serial_switch_routing_interfacesHandler(cfg),
	}
}
