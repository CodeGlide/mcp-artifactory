...
func Get_devices_serial_switch_routing_interfacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_devices_serial_switch_routing_interfacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_devices_serial_switch_routing_interfaces",
		mcp.WithDescription("List Layer 3 Interfaces For A Switch"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_devices_serial_switch_routing_interfacesHandler(cfg),
	}
}
