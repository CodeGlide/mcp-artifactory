...
func Get_networks_networkid_switch_stacks_switchstackid_routing_staticroutesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_stacks_switchstackid_routing_staticroutesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_stacks_switchStackId_routing_staticRoutes",
		mcp.WithDescription("List Layer 3 Static Routes For A Switch Stack"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_stacks_switchstackid_routing_staticroutesHandler(cfg),
	}
}
