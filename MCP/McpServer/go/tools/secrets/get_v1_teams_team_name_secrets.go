...
func Get_v1_teams_team_name_secretsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/secrets%s", cfg.BaseURL, team_name, queryString)
...
func CreateGet_v1_teams_team_name_secretsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_secrets",
		mcp.WithDescription("List top-level Secret Folders for User"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_secretsHandler(cfg),
	}
}
