...
func Get_v1_teams_team_name_secret_foldersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/secret_folders", cfg.BaseURL, team_name)
...
func CreateGet_v1_teams_team_name_secret_foldersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_secret_folders",
		mcp.WithDescription("List top-level Secret Folders for Team"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_secret_foldersHandler(cfg),
	}
}
