...
func Put_networks_networkid_appliance_firewall_onetoonenatrulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_appliance_firewall_onetoonenatrulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_appliance_firewall_oneToOneNatRules",
		mcp.WithDescription("Set The 1 1 NAT Mapping Rules For An MX Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_appliance_firewall_onetoonenatrulesHandler(cfg),
	}
}
