...
func Get_networks_networkid_appliance_warmspareHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_appliance_warmspareTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_appliance_warmSpare",
		mcp.WithDescription("Return MX Warm Spare Settings"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_appliance_warmspareHandler(cfg),
	}
}
