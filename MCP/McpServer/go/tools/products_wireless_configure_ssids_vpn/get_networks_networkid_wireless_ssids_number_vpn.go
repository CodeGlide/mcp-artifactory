...
func Get_networks_networkid_wireless_ssids_number_vpnHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_wireless_ssids_number_vpnTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_wireless_ssids_number_vpn",
		mcp.WithDescription("List The VPN Settings For The SSID"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_wireless_ssids_number_vpnHandler(cfg),
	}
}
