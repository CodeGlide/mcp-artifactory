...
func Get_v1_teams_team_name_current_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/current_user", cfg.BaseURL, team_name)
...
func CreateGet_v1_teams_team_name_current_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_current_user",
		mcp.WithDescription("Retrieve User details"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_current_userHandler(cfg),
	}
}
