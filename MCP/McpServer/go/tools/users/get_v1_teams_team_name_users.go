...
func Get_v1_teams_team_name_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/teams/%s/users%s", cfg.BaseURL, team_name, queryString)
...
func CreateGet_v1_teams_team_name_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_users",
		mcp.WithDescription("List all Users for a Team"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_usersHandler(cfg),
	}
}
