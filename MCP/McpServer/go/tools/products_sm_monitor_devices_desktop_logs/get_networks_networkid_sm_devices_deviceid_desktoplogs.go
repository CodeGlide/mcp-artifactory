...
func Get_networks_networkid_sm_devices_deviceid_desktoplogsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/sm/devices/%s/desktopLogs", cfg.BaseURL, networkId, deviceId)
...
func CreateGet_networks_networkid_sm_devices_deviceid_desktoplogsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_sm_devices_deviceId_desktopLogs",
		mcp.WithDescription("Return historical records of various Systems Manager network connection details for desktop devices."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_sm_devices_deviceid_desktoplogsHandler(cfg),
	}
}
