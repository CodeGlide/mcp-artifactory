...
func Put_networks_networkid_appliance_trafficshaping_rulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_appliance_trafficshaping_rulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_appliance_trafficShaping_rules",
		mcp.WithDescription("Update The Traffic Shaping Settings Rules For An MX Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_appliance_trafficshaping_rulesHandler(cfg),
	}
}
