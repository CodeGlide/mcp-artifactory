package tools

import (
	"context"
	"encoding/json"
	"fmt"
	"io"
	"net/http"

	"github.com/okta-privileged-access/mcp-server/config"
	"github.com/okta-privileged-access/mcp-server/models"
	"github.com/mark3labs/mcp-go/mcp"
)

func Delete_v1_teams_team_name_groups_group_name_users_user_nameHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		team_nameVal, ok := args["team_name"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: team_name"), nil
		}
		team_name, ok := team_nameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: team_name"), nil
		}
		group_nameVal, ok := args["group_name"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: group_name"), nil
		}
		group_name, ok := group_nameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: group_name"), nil
		}
		user_nameVal, ok := args["user_name"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: user_name"), nil
		}
		user_name, ok := user_nameVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: user_name"), nil
		}
		url := fmt.Sprintf("%s/v1/teams/%s/groups/%s/users/%s", cfg.BaseURL, team_name, group_name, user_name)
		req, err := http.NewRequest("DELETE", url, nil)
		if err != nil {
			return mcp.NewToolResultErrorFromErr("Failed to create request", err), nil
		}
		// Set authentication header based on auth type
		if cfg.BearerToken != "" {
			req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", cfg.BearerToken))
		}
		req.Header.Set("Accept", "application/json")

		resp, err := http.DefaultClient.Do(req)
		if err != nil {
			return mcp.NewToolResultErrorFromErr("Request failed", err), nil
		}
		defer resp.Body.Close()

		body, err := io.ReadAll(resp.Body)
		if err != nil {
			return mcp.NewToolResultErrorFromErr("Failed to read response body", err), nil
		}

		if resp.StatusCode >= 400 {
			return mcp.NewToolResultError(fmt.Sprintf("API error: %s", body)), nil
		}
		// Use properly typed response
		var result map[string]interface{}
		if err := json.Unmarshal(body, &result); err != nil {
			// Fallback to raw text if unmarshaling fails
			return mcp.NewToolResultText(string(body)), nil
		}

		prettyJSON, err := json.MarshalIndent(result, "", "  ")
		if err != nil {
			return mcp.NewToolResultErrorFromErr("Failed to format JSON", err), nil
		}

		return mcp.NewToolResultText(string(prettyJSON)), nil
	}
}

func CreateDelete_v1_teams_team_name_groups_group_name_users_user_nameTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_v1_teams_team_name_groups_group_name_users_user_name",
		mcp.WithDescription("Remove a User from a Group"),
		mcp.WithString("team_name", mcp.Required(), mcp.Description("(Required) The name of your Team")),
		mcp.WithString("group_name", mcp.Required(), mcp.Description("(Required) The name of a Group")),
		mcp.WithString("user_name", mcp.Required(), mcp.Description("(Required) The username for an existing User")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_v1_teams_team_name_groups_group_name_users_user_nameHandler(cfg),
	}
}
