...
func Get_v1_teams_team_name_groups_group_name_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["user_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("user_type=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/v1/teams/%s/groups/%s/users%s", cfg.BaseURL, team_name, group_name, queryString)
...
func CreateGet_v1_teams_team_name_groups_group_name_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_teams_team_name_groups_group_name_users",
		mcp.WithDescription("List all Users for a Group"),
		...
		mcp.WithString("user_type", mcp.Description("Only return Users of the specified type. Valid types: `human` and `service`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_v1_teams_team_name_groups_group_name_usersHandler(cfg),
	}
}
