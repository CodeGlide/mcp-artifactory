...
func Get_networks_networkid_switch_accesspoliciesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGet_networks_networkid_switch_accesspoliciesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_switch_accessPolicies",
		mcp.WithDescription("List The Access Policies For A Switch Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_switch_accesspoliciesHandler(cfg),
	}
}
