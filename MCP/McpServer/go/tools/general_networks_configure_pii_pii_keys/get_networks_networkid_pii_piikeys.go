...
func Get_networks_networkid_pii_piikeysHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/pii/piiKeys", cfg.BaseURL, networkId)
...
func CreateGet_networks_networkid_pii_piikeysTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_networkId_pii_piiKeys",
		mcp.WithDescription("List the keys required to access Personally Identifiable Information (PII) for a given identifier"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_networks_networkid_pii_piikeysHandler(cfg),
	}
}
