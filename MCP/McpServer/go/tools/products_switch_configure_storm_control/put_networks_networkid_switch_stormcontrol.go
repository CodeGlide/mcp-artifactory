...
func Put_networks_networkid_switch_stormcontrolHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePut_networks_networkid_switch_stormcontrolTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_networks_networkId_switch_stormControl",
		mcp.WithDescription("Update The Storm Control Configuration For A Switch Network"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_networks_networkid_switch_stormcontrolHandler(cfg),
	}
}
