...
func Get_tags_tag_name_media_recentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["min_tag_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("min_tag_id=%v", val))
		}
		if val, ok := args["max_tag_id"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("max_tag_id=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/tags/%s/media/recent%s", cfg.BaseURL, tag_name, queryString)
...
func CreateGet_tags_tag_name_media_recentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_tags_tag-name_media_recent",
		mcp.WithDescription("Get a list of recently tagged media."),
		mcp.WithString("count", mcp.Description("Max number of media to return.")),
		mcp.WithString("min_tag_id", mcp.Description("Return media before this `min_tag_id`.")),
		mcp.WithString("max_tag_id", mcp.Description("Return media after this `max_tag_id`.")),
		mcp.WithString("tag-name", mcp.Required(), mcp.Description("The tag name.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_tags_tag_name_media_recentHandler(cfg),
	}
}
