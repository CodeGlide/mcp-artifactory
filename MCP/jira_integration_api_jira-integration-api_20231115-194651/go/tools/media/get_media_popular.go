...
func Get_media_popularHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		url := fmt.Sprintf("%s/media/popular", cfg.BaseURL)
...
func CreateGet_media_popularTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_media_popular",
		mcp.WithDescription("Get a list of currently popular media."),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_media_popularHandler(cfg),
	}
}
