...
func Get_media_media_id_commentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/media/%s/comments", cfg.BaseURL, media_id)
...
func CreateGet_media_media_id_commentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_media_media-id_comments",
		mcp.WithDescription("Get a list of recent comments on a media object."),
		mcp.WithString("media-id", mcp.Required(), mcp.Description("The ID of the media resource.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_media_media_id_commentsHandler(cfg),
	}
}
