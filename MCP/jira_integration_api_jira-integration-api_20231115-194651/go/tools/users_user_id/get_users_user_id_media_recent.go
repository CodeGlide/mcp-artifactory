...
func Get_users_user_id_media_recentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		user_idVal, ok := args["user-id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: user-id"), nil
		}
		user_id, ok := user_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: user-id"), nil
		}
		...
		url := fmt.Sprintf("%s/users/%s/media/recent%s", cfg.BaseURL, user_id, queryString)
...
func CreateGet_users_user_id_media_recentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_user-id_media_recent",
		mcp.WithDescription("Get the most recent media published by a user."),
		...
		mcp.WithString("user-id", mcp.Required(), mcp.Description("The ID of a user to get information about, or **self** to retrieve information about authenticated user.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_users_user_id_media_recentHandler(cfg),
	}
}
