...
func Get_users_searchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/search%s", cfg.BaseURL, queryString)
...
func CreateGet_users_searchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_search",
		mcp.WithDescription("Search for a user by name."),
		mcp.WithString("q", mcp.Description("A valid tag name without a leading \\#. (eg. snowy, nofilter)")),
		mcp.WithString("count", mcp.Description("Max number of media to return.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_users_searchHandler(cfg),
	}
}
