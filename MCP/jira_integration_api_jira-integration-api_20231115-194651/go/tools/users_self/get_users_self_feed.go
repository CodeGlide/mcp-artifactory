...
func Get_users_self_feedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/self/feed%s", cfg.BaseURL, queryString)
...
func CreateGet_users_self_feedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_self_feed",
		mcp.WithDescription("See the authenticated user's feed."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_users_self_feedHandler(cfg),
	}
}
