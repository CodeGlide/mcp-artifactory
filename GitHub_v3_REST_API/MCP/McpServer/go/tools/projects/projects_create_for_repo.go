...
func Projects_create_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/projects", cfg.BaseURL, owner, repo)
...
func CreateProjects_create_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_projects",
		mcp.WithDescription("Create a repository project"),
		...
		mcp.WithString("body", mcp.Description("Input parameter: The description of the project.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Projects_create_for_repoHandler(cfg),
	}
}
