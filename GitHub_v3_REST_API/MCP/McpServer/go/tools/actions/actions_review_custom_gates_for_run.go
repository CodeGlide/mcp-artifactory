...
func Actions_review_custom_gates_for_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/deployment_protection_rule", cfg.BaseURL, owner, repo, run_id)
...
func CreateActions_review_custom_gates_for_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_runs_run_id_deployment_protection_rule",
		mcp.WithDescription("Review custom deployment protection rules for a workflow run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_review_custom_gates_for_runHandler(cfg),
	}
}
