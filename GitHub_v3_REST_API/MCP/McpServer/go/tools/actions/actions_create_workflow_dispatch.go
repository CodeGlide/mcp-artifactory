...
func Actions_create_workflow_dispatchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/workflows/%s/dispatches", cfg.BaseURL, owner, repo, workflow_id)
...
func CreateActions_create_workflow_dispatchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_workflows_workflow_id_dispatches",
		mcp.WithDescription("Create a workflow dispatch event"),
		...
		mcp.WithString("ref", mcp.Required(), mcp.Description("Input parameter: The git reference for the workflow. The reference can be a branch or tag name.")),
		mcp.WithObject("inputs", mcp.Description("Input parameter: Input keys and values configured in the workflow file. The maximum number of properties is 10. Any default properties configured in the workflow file will be used when `inputs` are omitted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_create_workflow_dispatchHandler(cfg),
	}
}
