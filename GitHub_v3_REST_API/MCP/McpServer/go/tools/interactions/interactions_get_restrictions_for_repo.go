...
func Interactions_get_restrictions_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/interaction-limits", cfg.BaseURL, owner, repo)
...
func CreateInteractions_get_restrictions_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_interaction-limits",
		mcp.WithDescription("Get interaction restrictions for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Interactions_get_restrictions_for_repoHandler(cfg),
	}
}
