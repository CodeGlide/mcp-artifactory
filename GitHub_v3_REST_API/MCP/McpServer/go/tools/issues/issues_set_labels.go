...
func Issues_set_labelsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/labels", cfg.BaseURL, owner, repo, issue_number)
...
func CreateIssues_set_labelsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_issues_issue_number_labels",
		mcp.WithDescription("Set labels for an issue"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_set_labelsHandler(cfg),
	}
}
