...
func Activity_list_stargazers_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/stargazers%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActivity_list_stargazers_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_stargazers",
		mcp.WithDescription("List stargazers"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_stargazers_for_repoHandler(cfg),
	}
}
