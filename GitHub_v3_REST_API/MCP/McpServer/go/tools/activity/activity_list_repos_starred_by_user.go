...
func Activity_list_repos_starred_by_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/starred%s", cfg.BaseURL, username, queryString)
...
func CreateActivity_list_repos_starred_by_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_starred",
		mcp.WithDescription("List repositories starred by a user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
		mcp.WithString("sort", mcp.Description("The property to sort the results by. `created` means when the repository was starred. `updated` means when the repository was last pushed to.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_repos_starred_by_userHandler(cfg),
	}
}
