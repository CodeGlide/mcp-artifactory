...
func Repos_set_status_check_contextsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_set_status_check_contextsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_branches_branch_protection_required_status_checks_contexts",
		mcp.WithDescription("Set status check contexts"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_set_status_check_contextsHandler(cfg),
	}
}
