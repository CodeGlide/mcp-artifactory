...
func Repos_add_status_check_contextsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_add_status_check_contextsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_branches_branch_protection_required_status_checks_contexts",
		mcp.WithDescription("Add status check contexts"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_add_status_check_contextsHandler(cfg),
	}
}
